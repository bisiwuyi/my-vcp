# PluginManager + VTPBroker 全链路整合优化方案 v2.0
**适用版本**：VCP ≥ 2.0  
**总开发周期**：9人日  
**核心原则**：100%生态兼容、无破坏性变更、开关可控、风险可回滚、操作零感知
...
**核心目标**：实现VTPBroker能力内置，消除跨进程同步开销
1. 代码移植：将`modules/vtbroker`核心逻辑（索引构建、语义匹配、API路由）移植到`PluginManager/src/modules/builtin_vtbroker`，去掉冗余的跨进程同步、事件订阅逻辑
2. 钩子对接：在PluginManager`pluginLoadSuccess`/`pluginUnload`/`pluginUpdate`钩子中新增元数据实时同步逻辑，插件加载完成即刻注入索引，零延迟生效
...
**验收标准**：插件热重载后元数据生效耗时<100ms，list_tools返回结果与原独立服务完全一致

---
### 阶段2：适配层兼容改造（2人日）
**核心目标**：上层调用无感知切换两种架构模式
1. 适配插件改造：更新`Plugin/vtbroker`逻辑，优先读取内置模块本地接口，开关关闭时回退到原HTTP调用逻辑
...
- 三层披露机制实现：
  ...
  2. 按需动态检索：Agent调用`vtbroker_search_tools`时仅返回与任务高度相关的1~3个工具完整Schema，单工具≤100Token
  3. 运行时纠错补全：工具调用错误时自动返回对应参数说明，≤50Token
- 效果：Token消耗降低40%+，工具调用准确率提升20%+
#### 3.2 插件注意点按需注入能力开发
...
**验收标准**：开启内置模块后，原有所有业务逻辑无任何异常

---

## 三、AdminPanel界面改造详细要求
| 改造位置 | 新增元素 | 样式要求 | 逻辑说明 |
| --- | --- | --- | --- |
...
| 全局配置页 | 新增「内置VTPBroker」配置分组 | 与现有配置分组样式一致 | 包含开关、端口配置，可直接切换架构模式 |
