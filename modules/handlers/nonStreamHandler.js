    ...
    do {
      ...
      if (toolCalls.length > 0) {
        ...
        const archeryLogs = await Promise.all(archeryCalls.map(async toolCall => {
          try {
            const result = await toolExecutor.execute(toolCall, clientIp, currentMessagesForNonStreamLoop, this.context.agentContext);
        ...
        const toolResults = await toolExecutor.executeAll(normalCalls, clientIp, currentMessagesForNonStreamLoop, this.context.agentContext);
