  ...
  async handle(req, res, firstAiAPIResponse) {
    ...
    while (recursionDepth < maxRecursion) {
      ...
      const archeryLogs = await Promise.all(archeryCalls.map(async toolCall => {
        try {
          const result = await toolExecutor.execute(toolCall, clientIp, currentMessagesForLoop, this.context.agentContext);
      ...
      // 执行普通调用
      const toolResults = await toolExecutor.executeAll(normalCalls, clientIp, currentMessagesForLoop, this.context.agentContext);
